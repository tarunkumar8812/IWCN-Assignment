...
//         let { title, fName, lName, email, phone, password, age, street, city, pincode } = req.body
...
//             INSERT INTO authors (authorId, title, fName, lName, email, phone, password, age, street, city, pincode, profilePhoto)
//             VALUES ( "${authorId}", "${title}", "${fName}", "${lName}", "${email.toLowerCase()}", "${phone}", "${password}", ${age}, "${street}", "${city}", "${pincode}", "${title} ${fName} ${lName}'s profile Photo" )
//         `)
...
// // ----------------------------- get author profile by their authorId -----------------------------
...
//         `) // if data not found empty array is returned
//         // if data found array of objects is returned
...
//         return res.status(200).json({ status: true, message: `${result[0][0].fName}'s Profile`, data: result[0] })
...
// // ----------------------------- update Author by their authorId -----------------------------
...
//             WHERE id = 8
//             `)
//         // WHERE isAdmin = 1 OR authorId = "${token?.authorId}"
...
//             return res
//                 .status(200)
//                 .json({ status: true, message: ` updated successfully`, data: result[0] })
//         }
//         return res
//             .status(200)
...
//             UPDATE authors
//             SET isDeleted = 1
//             WHERE authorId = "${token.authorId}" OR isAdmin = 1
//         `)
